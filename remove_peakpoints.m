...
function remove_peakpoints(spike_times,peak_values,varargin)
...
for i = 1:length(spike_times)
    %Remove the arrow at the spike time location by replacing with white arrow
    ...
    if xAnnotation(1) >= 0 && xAnnotation(1) <= 1 && yAnnotation(2) >= 0 && yAnnotation(1) <= 1
       hAnnotation = handle(annotation('arrow',xAnnotation,yAnnotation,'Tag','Peak Arrows','Color','w'));
    end
